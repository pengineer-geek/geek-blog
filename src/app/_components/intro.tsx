  ...
  return (
    <section className="mb-16">
      ...
      <div className="container py-8 md:py-10">
        <div className="grid w-full grid-cols-[84px_1fr] items-center gap-4 md:grid-cols-[96px_1fr] md:gap-6">
          ...
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-relaxed text-text/80 md:text-base">
      ...
      <div className="container mt-8 grid gap-6 md:grid-cols-2">
        ...
        <CategoryCard
          title="テック"
          items={[{ label: "開発した成果物についていろいろ", href: "https://example.com" }]}
        />
        <CategoryCard
          title="ウェルネス"
          items={[
      ...
      <ul className="space-y-2 text-text">
        {items.map((i) => (
          <li key={i.label} className="list-disc pl-5 marker:text-border">
            <Link href={i.href} className="text-link underline-offset-2 hover:text-accent hover:underline">
              {i.label}
            </Link>
          </li>
        ))}
